...
class Featurizer:
    ...
    def featurize(self, langs_target, langs_pivot, task_sizes):
        ...
        for pivot in ALL_LANG_CODES:
            l1 = []
            l2 = []
            for lang in langs_target:
                if pivot in langs_pivot:
                    m1 = self.precomputed_syntactic_distance[self.langs_list.index(pivot)][self.langs_list.index(lang)]
                    m2 = self.precomputed_type_overlaps[pivot][lang]
                else:
                    m1 = 0
                    m2 = 0
                l1.append(m1)
                l2.append(m2)
            syntactic_distance.append(l1)
            type_overlaps.append(l2)

        pivot_fts = [
            self.regression_feats.loc[langs_target].values,
        ]
        # print("*****************************************")
        # print(pivot_fts[0][1][1])
        # print("*****************************************")

        target_lang_fts = pivot_fts
        l=[]
        for lang in ALL_LANG_CODES:
            if lang in self.langs_list:
                m = task_sizes[self.langs_list.index(lang)]
            else:
                m = 0
            l.append(m)

        task_size = [l for i in range(len(langs_target))]
        finetuning_fts = create_finetuning_fts(target_lang_fts, task_size, langs_target)
        ...
        if self.pivot_features != "none":
            ...
            pivot_fts.append(np.array(finetuning_fts))

        # task_size = [task_sizes for i in range(len(langs_target))]

        pivot_fts.append(np.array(task_size).transpose(0, 1))

        pivot_fts = np.concatenate(pivot_fts, axis=1)
        # print(np.shape(pivot_fts))
        target_lang_in_pretrain = [1 if lang in self.langs_list else 0 for lang in langs_target]
        # print(np.array(target_lang_in_pretrain).reshape(-1, 1))
        scripts = get_target_lang_scripts(langs_target)
        # print(np.array(task_size).shape)
        pivot_fts = np.column_stack((pivot_fts, target_lang_in_pretrain))
        # print(pivot_fts[0, 250:])
        # print(np.shape(pivot_fts))

        pivot_fts = np.column_stack((pivot_fts, scripts))
        # print(pivot_fts[0, 250:])
        # print(np.shape(pivot_fts))
...
def create_finetuning_fts(target_lang_fts, task_size, langs_target):
    target_lang_ft = [[x[0] for x in t] for t in target_lang_fts]
    q=[]
    m, _ = np.shape(task_size)
    i = 0
    while i < m:
        q.extend(target_lang_ft)
        i = i + len(target_lang_ft)
    if i > m:
        k = i - m
        for j in range(k):
            q.pop()
    ft = np.dot(q, task_size)
    return ft

def get_target_lang_scripts(langs_target):
    list = []
    writing_script_list = [k for k,v in WRITING_SCRIPTS.items()]
    # print(writing_script_list)
    for lang in langs_target:
        l = np.zeros(len(writing_script_list), dtype = int)
        for k, v in WRITING_SCRIPTS.items():
            if lang in v:
                l[writing_script_list.index(k)] = 1
        list.append(l)
    return np.array(list)


# def target_pretraining_fts(langs_list, langs_target):
#     fts = []
#     for lang in langs_target:
#         f = np.zeros(len(langs_list), dtype = int)
#         if lang in langs_list:
#             f[langs_list.index(lang)] = 1
#         # print(f)
#         fts.append(f)
#     return fts
...
def data_combine(args):
    ...
    if args.use_all_langs:
        all_langs = set(MODEL2LANGS["tulrv6"]) | set(MODEL2LANGS["xlmr"])
    else:
        all_langs = set(langs_list)
    # print("*****************************************")
    # print(args.use_all_langs, args.scores_file, len(all_langs))
    # print("*****************************************")
    ...
    return langs_list, featurizer, X_array, Y_array, langs_pivot, langs_target, examples


def prepare_data(args):
    X_array, Y_array, langs_pivot, langs_target, examples = [],[],[],[], []
    if args.model_name == 'both':
        prev_score_file = args.scores_file

        # print("***********************************")
        # print(args.scores_file)
        # print("***********************************")

        if "TULR" in prev_score_file:
            args.model_name = "xlmr"
            args.scores_file = "./data/train/XLMR.json"
            langs_list, featurizer, X_array1, Y_array1, langs_pivot1, langs_tgts1, examples1 = data_combine(args)

            X_array.extend(X_array1)
            Y_array.extend(Y_array1)
            langs_pivot.extend(langs_pivot1)
            langs_target.extend(langs_tgts1)
            examples.extend(examples1)

            args.model_name = "tulrv6"
            args.scores_file = "./data/train/TULRv6.json"
            langs_list, featurizer, X_array1, Y_array1, langs_pivot1, langs_tgts1, examples1 = data_combine(args)

            X_array.extend(X_array1)
            Y_array.extend(Y_array1)
            langs_pivot.extend(x for x in langs_pivot1 if x not in langs_pivot)
            langs_target.extend(x for x in langs_tgts1 if x not in langs_target)
            examples.extend(x for x in examples1 if x not in examples)

        elif "XLMR" in prev_score_file:
            args.model_name = "tulrv6"
            args.scores_file = "./data/train/TULRv6.json"
            langs_list, featurizer, X_array1, Y_array1, langs_pivot1, langs_tgts1, examples1 = data_combine(args)
        ...
        args.model_name = "both"
        args.scores_file = prev_score_file


    else :
        langs_list, featurizer, X_array, Y_array, langs_pivot, langs_target, examples = data_combine(args)

    model = args.model_name
    # print("#######################################################")
    # print(langs_target)
    # print(len(langs_pivot))
    # print("#######################################################")
    # Reshape datasets
...
def create_validation_data(args):
    direct = "SumEval/data/train/"
    f = args.scores_file
    train_file = os.path.basename(f)
    test_file = direct+pred2test_files[train_file]
    args.scores_file = test_file
    prev_model = args.model_name
    args.model_name = "xlmr" if "XLMR" in f else "tulrv6"
    print("************************************")
    # print(train_file)
    print(test_file)
    # print(args.scores_file)
    print("************************************")
    _, _, _, _, _, X, Y, _, _, _ = prepare_data(args)
    args.scores_file = f
    args.model_name = prev_model
    return X, Y
...
def train_model(
    ...
    if error_method == "split":
        ...
        # X_train = X
        # Y_train = Y
        # X_test, Y_test = create_validation_data(args)
        # print("*****************************************")
...
def litmus_main(args):
    ...
    if args.mode:
        ...
        # pprint(ret_val["user-config-perfs"]['tgt_perfs'])

        # print("#######################################################")
        # for i in range(len(ret_val["user-config-perfs"]['tgt_perfs'])):
        #     print(len(ret_val["user-config-perfs"]['tgt_perfs'][i]))
        # print("#######################################################")
...
def parse_args(args):
    ...
    parser.add_argument(
        "--output", default=None, help="For all task+model system"
    )
    parser.add_argument(
